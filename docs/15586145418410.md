...
实例：
youtube-dl --proxy http://127.0.0.1:1087 -f 18 https://www.youtube.com/watch\?v\=qp159K0kh7E -o youtubu_test.mp4 

- --proxy http://127.0.0.1:1087 翻墙必备 
-  -f, --format FORMAT   视频/音频形式 分辨率    
-  -F        视频格式代码,查看"FORMAT SELECTION"获取所有信息
...
def load_movies(name_list):
    for url, name in name_list:
        os.system("youtube-dl --proxy http://127.0.0.1:1087  -f 22 {} -o {}".format(url, name))
